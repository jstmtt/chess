...
import "./styles.css";
import { fetchRapidHistory, fetchGamesBetween } from "./api/chessApi";
      ...
      <div className="stat-main-row">
        <div>
          ...
          <div className="stat-badges">
            {/* FEATURE 1: CURRENT PEAK HIGHLIGHT */}
            {best && (
              <div className={`stat-badge peak ${value === best ? 'current-peak' : ''}`}>
            ...
            {profile?.streak?.count >= 3 && (
              <div className={`stat-badge streak ${profile.streak.type}`}>
                <span className="badge-icon">
                  {profile.streak.type === 'win' ? '🔥' : profile.streak.type === 'loss' ? '❄️' : '➖'}
                </span>
                <span>{profile.streak.count} {profile.streak.type === 'win' ? 'Win' : profile.streak.type === 'loss' ? 'Loss' : 'Draw'} Streak</span>
              </div>
            )}
...
export default function Chess() {
  ...
  // Load Rivalry Data
  useEffect(() => {
    // WAIT for the main chart data to finish loading first
    if (loading) return;
    ...
    try {
      const entries = await Promise.all(
        PLAYERS.map(async (player) => {
          ...
          const popupUrl = encodeURIComponent(`https://www.chess.com/callback/user/popup/${player.username}`);
          const onlinePromise = fetch(`https://corsproxy.io/?url=${popupUrl}`, { cache: "no-store", headers: { 'Accept': 'application/json' } })
            .then(res => res.json())
            .then(data => data?.onlineStatus === 'online' || data?.onlineStatus === 'onlive')
            .catch((err) => {
              console.warn("Failed to fetch online status", err);
              return false;
            });

          const [historyPayload, stats, profileData, isOnline] = await Promise.all([
            historyPromise,
            statsPromise,
            profilePromise,
            onlinePromise,
          ...
          // Safely extract streak and history regardless of the payload shape
          const rawHistory = Array.isArray(historyPayload) ? historyPayload : (historyPayload?.history || []);
          const streak = historyPayload?.streak || null;
